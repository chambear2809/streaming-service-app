...
8. When the user wants PostgreSQL DB monitoring in Splunk Observability Cloud, treat it as a follow-on collector change, not as part of the base collector bootstrap itself.
   ...
   - The canonical deploy script can now install or reuse the base Splunk OTel Collector when `--splunk-otel-mode` is set, but PostgreSQL DB monitoring is still a separate Helm values overlay on that collector release.
