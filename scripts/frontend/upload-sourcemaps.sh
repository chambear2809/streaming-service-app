...
if [[ -z "${SPLUNK_SOURCEMAP_UPLOAD_TOKEN}" && ( -n "${SPLUNK_RUM_ACCESS_TOKEN}" || -n "${SPLUNK_RUM_SECONDARY_ACCESS_TOKEN}" ) ]]; then
  fail "Browser RUM tokens are for Browser RUM only. Set SPLUNK_ACCESS_TOKEN or SPLUNK_SOURCEMAP_UPLOAD_TOKEN for sourcemap upload."
fi
